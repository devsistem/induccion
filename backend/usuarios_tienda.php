...
loadClasses('BackendUsuario',  'Usuario');
global $BackendUsuario, $Vendedor, $Usuario;
...
switch ($accion) {
 case 'eliminar':
  if(is_array($_POST['arrSeleccion'])) foreach($_POST['arrSeleccion'] as $idx) {
 	  $Usuario->eliminar($idx);
   }
 break;

 case 'publicar':
	   $Usuario->publicar($_POST['id'], $_POST['campo']);
 break;

 case 'estado':
	   $Usuario->estado($_POST['id'], $_POST['campo']);
 break;
}

// solo muestra sus vendeores
$result = $Usuario->obtener_all(null, null, null, null, null, null, ACTIVO, null, null);
