...
// 1002493714
// 01/09/2015 20:36:30
// 02/03/2016 16:42:26 fecha manual
ob_start();
...
switch($action) {
    ...
    if(strlen($cuen) !=  10 ) {
		...
		if(strlen($cedula) !=  10 ) {
			 $str_errors  .= "La cedula no es válida";
			 $css_cedula = "error";
			 $errores++;
		}
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyArf275rLC1DR8WMoL7_KhER619zlFlUQ8&libraries=places"></script>
      ...
      <input type="hidden" id="pais_mapa" name="pais_mapa" value="Ecuador">	
		 ...
		 <div style="padding:20px">
		  <div class="control-group" align="right">
				...
				<input required="required" class="texto_cuen" id="registro_cuen" placeholder="CUEN" name="registro[cuen]" style="width:20%"  maxlength="10" type="text" value="<?=$_POST['registro']['cuen']?>" onBlur="es_cuen_valida();">
		 ...
		 <hr size="2" color="#737373">			
			     ...
			     <tr>
			      ...
			      <td width="50%" style="padding-left:10px">
			      	<!--
							...
							<input required="required" class="form-control" style="width:100%" id="registro_dni" name="registro[dni]" style="width:20%"  maxlength="15" type="text" value="<?=$_POST['registro']['dni']?>">
			...
			<table width="100%" border="0">
			 <tr>
			  <td width="48%" valign="top">
			    ...
			    <table cellpadding="10">
			     <tr>
			      ...
			      <td style="padding-left:10px">
							<input  class="form-control" id="register_name" name="registro[parroquia]" style="width:90%"  maxlength="150" type="text" value="<?=$_POST['registro']['parroquia']?>">
			      </td>
			     </tr>
					 <tr>
					  <td colspan="2" height=10"></td>
			      ...
			      <td style="padding-left:10px">
						<input class="form-control" id="registro_barrio" name="registro[barrio]" style="width:90%"   maxlength="150" type="text" value="<?=$_POST['registro']['barrio']?>">
			      </td>
			     </tr>
					 <tr>
					  <td colspan="2" height=10"></td>
			     ...
			     <tr>
			      ...
			      <td style="padding-left:10px">
							<textarea  class="form-control" id="registro_cliente_referencia" name="registro[cliente_referencia]" maxlength="250" style="width:90%"> <?=$_POST['registro']['cliente_referencia']?></textarea>
			      </td>
			     </tr>
			    </table>
			  </td>
			  ...
			  <td width="52%" valign="top">
			  			...
			  			<input type="hidden" id="latitude"   name="latitude" value="<?=$arrActual['latitude']?>" 	maxlength="10"  size="20">		
							<input type="hidden" id="longitude"  name="longitude"  value="<?=$arrActual['longitude']?>"  maxlength="10" size="20">
  		...
  		<hr size="2" color="#737373">			
			...
			<select id="registro_modelo" name="registro[modelo]"  class="select_cocina">
		 			 <option value="">MODELO</option>
					 <?php	
						for($i=1; $i <= $filas_modelos_cocina; $i++) {
							$items_modelos_cocina = @mysql_fetch_array($result_modelos_cocina); ?>
					 		 <option value="<?=$items_modelos_cocina['nombre']?>"   <?=($_POST['registro']['modelo']==$items_modelos_cocina['nombre']) ? 'selected' : ''?>><?=$items_modelos_cocina['nombre']?></option>
					 	<?php } ?>	 
			</select>
			<?php */ ?>
				 ...
				 <tr>
				 	<td align="left" width="20%">
					 		...
					 		<tr>
					 		 ...
					 		 <td><input class="form-control" id="registro_cuota_entrada" name="registro[cuota_entrada]" style="width:50px;height:40px"   maxlength="15" type="text" value="<?=$_POST['registro']['cuota_entrada']?>"></td>
					 		</tr>
					 		</table>
				 	...
				 	<select id="registro_forma_pago" name="registro[forma_pago]"  class="select_cocina">
		 			 <option value="">FORMA PAGO</option>
						 <option value="CREDITO DEL ESTADO" <?=($_POST['registro']['forma_pago']=="CREDITO DEL ESTADO") ? 'selected' : ''?>>CREDITO DEL ESTADO</option>
					...
					<select name="registro[cuotas]" id="registro[cuotas]" class="select_cocina"  style="width:100px; <?=$css_imagen_cuotas?>">
						 ...
						 <option value="72" <?=($_POST['registro']['cuotas']=="72") ? 'selected' : ''?>>72 Meses</option>
						 <option value="60" <?=($_POST['registro']['cuotas']=="60") ? 'selected' : ''?>>60 Meses</option>
						 <option value="48" <?=($_POST['registro']['cuotas']=="48") ? 'selected' : ''?>>48 Meses</option>
						 <option value="36" <?=($_POST['registro']['cuotas']=="36") ? 'selected' : ''?>>36 Meses</option>
						 <option value="24" <?=($_POST['registro']['cuotas']=="24") ? 'selected' : ''?>>24 Meses</option>
						 <option value="12" <?=($_POST['registro']['cuotas']=="12") ? 'selected' : ''?>>12 Meses</option>
					</select>  
				 	</td>
					 ...
					 <div style="padding:0px">	
						...
						<input type="checkbox" name="promocion" id="registro_promocion" value="1" <?=($_POST['promocion']==1) ? 'checked' : ''?> />
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<span class="titulo_form_24">Medidor 220</span>
						<input type="checkbox" name="medidor220" id="registro_medidor220" value="1" <?=($_POST['medidor220']==1) ? 'checked' : ''?> />
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<span class="titulo_form_24">Circuito interno</span>
						<input type="checkbox" name="circuito_interno" id="registro_circuito_interno" value="1"  <?=($_POST['circuito_interno']==1) ? 'checked' : ''?> />
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<span class="titulo_form_24">Ducha electrica</span>
						<input type="checkbox" name="ducha_electrica" id="registro_ducha_electrica" value="1"  <?=($_POST['ducha_electrica']==1) ? 'checked' : ''?> />
					 </div>
			 		</td>
			  </tr>
			 <tr>
			  ...
			  <tr>
			   <td colspan="4" align="center">
				...
				<select name="registro[horario_recepcion]" id="registro_horario_recepcion" class="select_horario">
					 <option value="De lunes a sabado de 07h00 a 14h00" <?=($_POST['registro']['horario_recepcion']=="De lunes a sabado de 07h00 a 14h00") ? 'selected' : ''?>>De lunes a sábado de 07h00 a 14h00</option>
					 <option value="Solo lunes a viernes de 07h00 a 14h00" <?=($_POST['registro']['horario_recepcion']=="Solo lunes a viernes de 07h00 a 14h00") ? 'selected' : ''?>>Solo lunes a viernes de 07h00 a 14h00</option>
					 <option value="Solo sabados de 07h00 a 14h00" <?=($_POST['registro']['horario_recepcion']=="Solo sabados de 07h00 a 14h00") ? 'selected' : ''?>>Sólo sábados de 07h00 a 14h00</option>
				</select>
